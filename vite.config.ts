...
function generationProxyPlugin(env: Record<string, string>): PluginOption {
  return {
    ...
    configureServer(server) {
      // Local-dev convenience only: a repo-root .env GEMINI_API_KEY lets the built-in Gemini
      // provider work without pasting a key. Every other provider is bring-your-own-key.
      const mw = createGenerationMiddleware({ GEMINI_API_KEY: env.GEMINI_API_KEY });

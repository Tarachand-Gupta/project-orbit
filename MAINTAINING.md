...
contributor PR ──▶ dev (default branch, CI must pass) ──▶ merge dev → main
                              ...
                              (macOS .app + Linux build             (projectorbitai.vercel.app)
...
| Where | What | Notes |
| --- | --- | --- |
| local `.env` | `GEMINI_API_KEY` | gitignored; dev-middleware convenience only, and **baked into locally-packaged .apps** — never distribute a locally-built .app |
| Vercel env | **nothing, by policy** | Enforced in code: `api/generate.ts` ignores provider keys unless `ALLOW_SERVER_KEYS=1` is *also* set, so a stray `GEMINI_API_KEY` can't drain silently. Without a key `/api/generate` returns "add your own key in the ⚙ settings". |

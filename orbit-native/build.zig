...
// No machine-specific path in a public repo. Provide the Native SDK checkout with
// -Dnative-sdk-path=<path> or the NATIVE_SDK_PATH env var (CI passes the flag). See
// orbit-native/CLAUDE.md for the install command.
const default_native_sdk_path = "";
const app_exe_name = "orbit-native";
...
pub fn build(b: *std.Build) void {
    ...
    const native_sdk_path = b.option([]const u8, "native-sdk-path", "Path to the Native SDK framework checkout") orelse
        b.graph.environ_map.get("NATIVE_SDK_PATH") orelse
        default_native_sdk_path;
    if (native_sdk_path.len == 0) {
        std.log.err(
            "Native SDK path not configured. Pass -Dnative-sdk-path=<path> or set NATIVE_SDK_PATH.\n" ++
                "  npm install -g @native-sdk/cli\n" ++
                "  zig build -Dnative-sdk-path=\"$(npm root -g)/@native-sdk/cli\"",
            .{},
        );
        std.process.exit(1);
    }

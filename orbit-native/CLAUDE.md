...
- `build.zig` needs the Native SDK checkout via `-Dnative-sdk-path=...` or the `NATIVE_SDK_PATH`
  env var (no machine-specific default in the repo). Missing → the build fails fast with the
  install command. CI passes `-Dnative-sdk-path="$(npm root -g)/@native-sdk/cli"`.

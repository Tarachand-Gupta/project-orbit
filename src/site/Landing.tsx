 ...
 * jungle/sky palette, the pixel wordmark — and the hero is the game's thesis: an AI Create bar
...
/** The bring-your-own-key AI providers to showcase (everything real except the offline "local"). */
const SHOWCASE_PROVIDERS = PROVIDERS.filter((p) => p.id !== "local").map((p) => p.label);
    ...
    <div className="landing min-h-screen text-white overflow-x-hidden">
      ...
      <div className="relative max-w-[1060px] mx-auto px-5 sm:px-8">
        ...
        <nav className="flex items-center justify-between py-6">
          <span className="landing-wordmark text-lg sm:text-xl select-none">
            PROJECT ORBIT <span className="text-sky-300">AI</span>
          </span>
        ...
        <header className="flex flex-col items-center text-center pt-10 sm:pt-16 pb-14 gap-7">
          <span className="glass rounded-full px-4 py-1.5 text-xs sm:text-sm text-sky-100 tracking-wide" data-testid="landing-ai-badge">
            ✦ AI-powered sandbox · bring your own key
          </span>
          <h1 className="landing-display text-4xl sm:text-6xl leading-[1.05] max-w-[760px]">
            Type it.
            <br />
            <span className="text-sky-200">AI builds it.</span>
          </h1>
          <p className="text-white/75 text-base sm:text-lg max-w-[640px]">
            <b className="text-white">Project Orbit AI</b> is an open-source sandbox where an AI model turns your
            words into real, physics-simulated objects. Describe anything — a supercar, the Taj Mahal, a helicopter —
            and it spawns into a low-poly world you can walk and drive through. <b className="text-white">Bring your
            own key</b> from any major provider; it stays in your browser.
          </p>
          <TypingPrompt />
        ...
        {/* Bring your own key — the AI providers */}
        <section className="py-12" data-testid="landing-providers">
          <h2 className="landing-display text-2xl sm:text-3xl text-center mb-3">Powered by your AI, your key</h2>
          <p className="text-white/70 text-center max-w-[650px] mx-auto mb-8 text-sm sm:text-base">
            Project Orbit AI is built on the Vercel AI SDK, so it's provider-agnostic. Pick a provider, paste your
            own API key (or a base URL for anything OpenAI-compatible), choose a model — and create. Your key stays
            in your browser and is sent only to the provider you pick; the deployment ships no keys of its own.
          </p>
          <div className="flex flex-wrap justify-center gap-2.5 max-w-[740px] mx-auto">
            {SHOWCASE_PROVIDERS.map((label) => (
              <span key={label} className="glass rounded-full px-4 py-2 text-sm text-white/85">{label}</span>
            ))}
          </div>
          <p className="text-white/45 text-xs text-center mt-6">
            No key? Run fully offline with the built-in template engine — instant objects, no network.
          </p>
        </section>
        ...
        <section className="py-12">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              ["⚡", "AI or offline", "Bring your own key from OpenAI, Anthropic, Gemini, Groq, and more — or run the deterministic template engine instantly with no key at all."],
        ...
        <section className="py-14 text-center">
          <div className="glass glass-strong rounded-3xl px-6 py-10 max-w-[760px] mx-auto flex flex-col items-center gap-4">
            ...
            <p className="text-white/75 max-w-[560px]">
              React Three Fiber, Rapier physics, Zustand, and the Vercel AI SDK — 160+ unit tests and a Playwright

...
const PAGES: Array<{ title: string; body: React.ReactNode }> = [
  {
    title: "Welcome to Project Orbit",
    body: (
  ...
  },
  {
    ...
    body: (
      <ul className="space-y-2 list-disc pl-4">
        ...
        <li>
          <b>Tune</b> — click any object to open its controls: top speed, size, mass, colors,
          rotor speed… changes apply live, mid-drive.
        </li>
        <li>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="text-white/50 text-left">
              ...
              <th className="pb-1 pr-2 font-medium">How</th>
          ...
          <tbody className="text-white/80">
            {(
              ...
              <tr key={p} className="border-t border-white/10">
                <td className="py-1.5 pr-2 font-medium">{p}</td>
                <td className="py-1.5 pr-2 font-mono text-[10px]">{how}</td>
                <td className="py-1.5 text-white/60">{note}</td>
              </tr>
            ))}
          </tbody>
        </table>
        <p className="text-white/60 text-xs">
          <b>Custom (OpenAI-compatible)</b> works with any endpoint that speaks the OpenAI chat
          API — Groq, Together, a local llama.cpp/Ollama server… Enter the base URL, a model
          name, and your key.
        </p>
      </div>
    ),
